...
\begin{enumerate}[1.]
  ...
  \item $\bm{Vec}_\mathbb F$ is the category where vector spaces over field $\mathbb F$ are the objects,
    and linear transformations are the morphisms.
  \item $\bm{Gr}$ is the category of directed graphs, where $Ob_{\bm{Gr}} := \{\text{Vertex}, \text{Arrow}\}$,
...
\begin{example}[Natural Transformations in Sets]
  Consider the categories $\bm 1$ and $\bm{Set}$.
  Since $\bm 1$ has only one object $1$ and one morphisms $id_1$, then each functor
  $F:\bm 1 \to \bm{Set}$ is completely defined by $F(1) = S$, where $S$ is a set, and,
  ...
  functor that $F(1)$ is equal to set $A$. Note that, in a sense, every functor
  $F_A$ is the ``same'' as the set $A$ itself. In a strange way, by going a level higher
  in abstraction we ended up going inside the category $\bm{Set}$.

  Next, we want to define a natural transformation $\alpha: F_A \to F_B$.
  By definition, there are two criteria to satisfy. First, for every object $1 \in Ob_{\bm 1}$ we have
  $\alpha_1:F_A(1) \to F_B(1)$ such that $\alpha_1 \in Mor_{\bm{Set}}(F_A(1),F_B(1))$.
  Since the only object in $\bm 1$ is $1$, we only need to worry about $\alpha_1$.
  Secondly, we need that for every $f \in Mor_{\bm 1} (1,1)$, it's true that
  \begin{displaymath}
    F_A(f) \fatsemi \alpha_1 = \alpha_1 \fatsemi F_B(f).
  \end{displaymath}
  Again, since the only morphism in $\bm 1$ is $id_1$, we only need to worry that
  \begin{displaymath}
    F_A(id_1) \fatsemi \alpha_1 = \alpha_1 \fatsemi F_B(id_1).
  \end{displaymath}

  Note that $F_A(id_1) = id_A$ and $F_B(id_1) = id_B$, and by the definition
  of identity, we have that for $g \in Mor(A,B)$, $id_A \fatsemi g = g \fatsemi id_B$.
  Therefore, the second condition is trivially satisfied. We just need to worry
