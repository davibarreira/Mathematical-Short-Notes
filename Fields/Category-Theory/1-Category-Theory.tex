...
\begin{example}[Category of Spans in $\mathcal C$]
  ...
  $X_{A,B}:\underline{2} \to \mathcal C$, where $X_{A,B}(1) = A$ and $X_{A,B}(2)= B$.
  ...
  Thus, if $F\circ i = X_{A,B}$, then the diagram above commutes and $F(1) = A, F(2) = B$, which means
  ...
  \begin{displaymath}
    Ob_{\mathcal C_{/ X}}:=\{
      F: \underline{2}^\lhd \to \mathcal C \ | \ F \circ i = X_{A,B}
    \}.
  \end{displaymath}
  \begin{displaymath}
    Mor_{\mathcal C_{/X}}(F,G):=\{
      \alpha: F \Rightarrow G \ | \ \alpha \diamond i = id_{X_{A,B}}
    \}.
  \end{displaymath}
  ...
  is restricted to $F\circ i = X_{A,B}$,
  but not why $\alpha \diamond i = id_{X_{A,B}}$ in the morphisms.
