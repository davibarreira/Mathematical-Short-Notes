...
\begin{example}[Natural Transformations in Sets]
  ...
  This is only a sketch proof, since we would still need to prove that such
  functor $I$ exists and is well-defined.
...
\section{Applied Category Theory}
...
between categories are graph homomorphisms.
...
\begin{definition}[Path Functor]
  ...
  graph homomorphisms $f:G \to G'$ to $Path(f):Path(G)\to Path(G')$.
\end{definition}

An example of a graph an it's path graph is again the graph in figure \ref{fig:2Catsimple}
...
For example,
consider the graph (diagram) in the figure \ref{fig:Square}. How can we complete this graph
in order to make it a category?
% First, we consider every vertex an object of the category and
% every arrow a morphism. Next, we could add loops in all vertices in order to
% define the identity morphisms. We are still missing the compositions $f \fatsemi h$
% and $$
...
When drawing diagrams (graphs) to represent categories, we sometimes need to explicitly
state that some composition of morphisms are equivalent. This can be formalized
in what is called by \citet{spivak2014category} as \textit{path equivalence declaration}.
...
\begin{definition}[Schema \citep{spivak2014category}]
  A database schema is a tuple $(G, \simeq)$ consisting of a graph $G$ and a set of PED's, also
