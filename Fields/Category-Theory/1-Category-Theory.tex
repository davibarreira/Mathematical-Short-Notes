...
\subsection{Categorical Product}
...
\begin{definition}[Categorical Product]
  Let $A,B$ be objects ina  category $\mathcal C$.
\end{definition}
...
\begin{example}[Natural Transformations in Sets]
  Consider the categories $\bm 1$ and $\bm{Set}$. We wish to
  construct the category $\bm{Set}^{\bm 1}$, i.e. of functors from $\bm 1$
  to $\bm{Set}$. Since $\bm 1$ has only one object $1$ and one morphisms $id_1$, then each functor
  ...
  Since $\alpha_1$ must be a morphism of $\bm{Set}$, then $\alpha_1$ is a function
  from $A$ to $B$.
  ...
  Considering the category $\bm{SmCat}$ of small categories, we can actually prove
  that $\bm{Set}$ and $\bm{Set}^{\bm 1}$ are isomorphic.

  Consider the functor
  $I:\bm{Set}^{\bm 1} \to \bm{Set}$ where $I(F_A) = A \in \bm{Set}$ and
